...
In addition to the pilot table, the repository now contains a reproducible set of exact-ideal single-shot visualizations for ZDT1, ZDT2, and ZDT3 in two objectives and for DTLZ1, DTLZ2, and DTLZ7 in three objectives.  These runs use population size $60$ and seed $2027$; ZDT1/2/3, DTLZ1, and DTLZ2 use $30{,}000$ function evaluations, whereas DTLZ7 uses $60{,}000$ evaluations in order to improve convergence on the disconnected front.  The three-objective plots use the fixed perspective projection employed in the earlier drafts (elevation $30^\circ$, azimuth $-60^\circ$), and the plotting order has been adjusted so that the true Pareto-front reference cloud is rendered as a light background layer while the IR2-EMOA approximation points remain visually in the foreground.  The corresponding objective vectors, metadata, and plotting scripts are included in the repository for exact reproduction.

\subsection{Single-shot gallery}

Figure~\ref{fig:single-shot-gallery} collects the six current exact-ideal single-shot plots used to visually verify the implementation and the plotting pipeline.  The three ZDT cases are shown in two objectives, whereas DTLZ1, DTLZ2, and DTLZ7 are shown in three objectives.  All runs use a population size of $60$ and seed $2027$; the budget is $30{,}000$ evaluations except for DTLZ7, which uses $60{,}000$ evaluations.
...
\includegraphics[width=0.48\linewidth]{figures/single_shot_zdt1_2d_exact_ideal_30000evals.png} &
\includegraphics[width=0.48\linewidth]{figures/single_shot_zdt2_2d_exact_ideal_30000evals.png}\\[1mm]
\textbf{ZDT3 (2D), 30k evals} & \textbf{DTLZ1 (3D), 30k evals}\\[-1mm]
\includegraphics[width=0.48\linewidth]{figures/single_shot_zdt3_2d_exact_ideal_30000evals.png} &
\includegraphics[width=0.48\linewidth]{figures/single_shot_dtlz1_3d_exact_ideal_30000evals.png}\\[1mm]
\textbf{DTLZ2 (3D), 30k evals} & \textbf{DTLZ7 (3D), 60k evals}\\[-1mm]
\includegraphics[width=0.48\linewidth]{figures/single_shot_dtlz2_3d_exact_ideal_30000evals.png} &
\includegraphics[width=0.48\linewidth]{figures/single_shot_dtlz7_3d_exact_ideal_60000evals.png}
\end{tabular}
\caption{Gallery of reproducible exact-ideal single-shot IR2-EMOA runs.  The light gray point clouds indicate the sampled true Pareto front, the darker points indicate the final IR2-EMOA approximation set, and the star marks the exact ideal point used by Integral R2.  The corresponding objective vectors and metadata are included in the repository.}
...
The accompanying source package contains the 2-D/3-D Integral R2 deletion-loss implementation, IR2-EMOA, the benchmark and plotting scripts, and all seeds and single-shot result files used in the present draft.
